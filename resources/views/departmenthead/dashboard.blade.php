...
@extends("departmentheadlayouts.theme")
 ...
 <div class="pagetitle">
      ...
      <div class="alert alert-primary" style="background-color:#1E2F97 ;color: white;">You are logged in as {{{ isset(Auth::user()->name) ? Auth::user()->name : Auth::user()->  email }}} | Approval pending :  
         ...
         <a href="{{route('department.document-list-approval-pending')}}" style="color: white;">{{$documentpending}} <span class="badge bg-warning">View</span></a> | Trash:  <a href="{{route('department.document-trash-list')}}" style="color: white;">{{$trashcount}} <span class="badge bg-danger">View</span></a> </div>
    </div><!-- End Page Title -->
    </div><!-- End Page Title -->
  ...
  <h5 class="card-title">Advance Filter</h5>
       ...
       <div class="alert alert-success alert-dismissible fade show" role="alert">
              ...
              <!-- Vertical Form -->
                 ...
                 <form class="row g-3" method="GET" action="{{ route('department.datewise-filter-search') }}" autocomplete="off">
